...
public class AlertRESTController {
    ...
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<List<Alert>> getAlertsByProductAndDate(@RequestParam @NotNull Long productId, @RequestParam @NotNull @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate availableOnDate) {
        log.trace("getAlertsByProductAndDate");
        return ResponseEntity.ok(alertService.findAlertsByProductAndDate(productId,availableOnDate));
    }

    @GetMapping("/byUserAndInterval")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity<List<Alert>> getAlertsByUserAndInterval(@RequestParam @NotNull Long userId, @RequestParam @NotNull @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate fromDate, @RequestParam @NotNull @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate toDate) {
        log.trace("getAlertsByUserAndInterval");

        return ResponseEntity.ok(alertService.findAlertsByUserAndInterval(userId, fromDate, toDate));
    }
